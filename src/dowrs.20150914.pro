...
; @keyword First {in}{optional}{type=boolean} increase initial Z4 offsets
...
; COMMENTED OUT THE INITIAL Z11
; last modified by J. M. Hill - 02-APR-2015
;        have /FIRST introduce some negative Z4
; last modified by J. M. Hill - 08-JUL-2015
;        make dofpia_wrs all lower case
; last modified by KS  -  09-JUL-2015
;        use WRSMode flag in lbcfpia_read_cfg to use different path
; last modified by J. M. Hill - 14-SEP-2015
;        version to read out a lower region on the CCD chip
;        comment out the keyboard query so it will run from a script (??)
;
...
; Main Program
Pro DoWRS, Sim=Sim, X2=X2, RedOnly=RedOnly, BlueOnly=BlueOnly, $
...
If DoRed EQ 0 AND DoBlue EQ 0 Then Begin
   Print, "Neither side of LBC is authorized - stopping DOWRS."
   Return ; exit program
EndIf
...
; Send initial focus (Z4) to mirrors to bias in extrafocal direction
; Added by JMH 20150402
If Keyword_Set(First) Then Begin 
; THESE VALUE ARE HARDWIRED HERE. THEY SHOULD BE IN PARAMETER FILE.
   RedFocus = -3000.0
   BlueFocus = -3000.0
Endif Else Begin
   RedFocus = 0.0
...
If DoRed GE 1 AND RedFocus NE 0.0 Then Begin
   Print, "Sending ",RedFocus,"nm Focus (Z4) to RED channel"
    SendSph3, RedFocus, /Red, Sim=Sim, Logfile=Logfile, WRSMode=1
Endif
If DoBlue GE 1 AND BlueFocus NE 0.0 Then Begin
    Print, "Sending ",BlueFocus,"nm Focus (Z4) to BLUE channel"
    SendSph3, BlueFocus, Sim=Sim, Logfile=Logfile, WRSMode=1
Endif
...
;
    KeyString = ""
; We commented this out so dowrs can be run from a shell script.
