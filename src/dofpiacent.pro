;+
; DOFPIACENT - First attempt to make automatic IA on LBC science chips
; created by Andrea Baruffolo - January 2009
;
;
; The procedure for semi-automatic Image Analysis on LBC focal plane is named
; 'dofpia.pro' and is located in '~lbcobs/LBCFPIA/lbcfpia/src/'.
; To run it:
; - do a 'source ~lbcobs/LBCFPIA/lbcfpia/setup.csh' (to set IDL_PATH and LBCFPIA_HOME)
...
; last modified by J. M. Hill - 06-SEP-2014
;                (Add 2 sec WAIT before playing OB to account for
;                cases where no pupils are found - when there are
;                intermittent clouds that make no pupils in iteration N.)
; SPECIAL VERSION TO USE CHIP 2 NEAR ROTATOR CENTER
...
; Main Program
Pro DoFPIAcent, Sim=Sim, X2=X2, RedOnly=RedOnly, BlueOnly=BlueOnly, $
...
;
cfg = lbcfpia_read_cfg(ErrCode=ErrCode,ErrMsg=ErrMsg)
...
If DoRed EQ 0 AND DoBlue EQ 0 Then Begin
   Print, "Neither side of LBC is authorized - stopping DOFPIA."
   Return ; exit program
EndIf
...
If Not KeyWord_Set(Sim) Then Begin
  If (DoBlue EQ 0) AND (DoRed GE 1) Then Begin
      Spawn, HomeDir+"/src/load-obcent-red.pl", Result, ErrResult
  EndIf  
  If (DoBlue GE 1) AND (DoRed EQ 0) Then Begin
      Spawn, HomeDir+"/src/load-obcent-blue.pl", Result, ErrResult
  EndIf
  If (DoBlue GE 1) AND (DoRed GE 1) Then Begin
      Spawn, HomeDir+"/src/load-obcent.pl", Result, ErrResult
  EndIf
