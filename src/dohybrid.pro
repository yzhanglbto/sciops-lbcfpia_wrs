;+
; DOHYBRID
; adapted from DOFPIA created by Andrea Baruffolo - January 2009
; modified September 2016 to use WRS followed by LBCFPIA.
;
;
; The procedure for semi-automatic Image Analysis on LBC focal plane is named
; 'dofpia.pro' and is located in '~lbceng/WRS/src/'.
; To run it:
; - do a 'source ~lbceng/WRS/setup.csh' (to set IDL_PATH and LBCFPIA_HOME)
; - cd to a writeable directory (for log files)
; - run IDL by typing 'idl'
; - at IDL prompt type 'dohybrid'
;
;
; In a nutshell, the procedure (DOHYBRID):
;
; from the procedure (DOWRS):
; make_wrs2
; 0- reads config file lbcwrs.cfg
; 0.5- checks that LBC is authorized
; 1- sends initial +600mm sph3 offset to both mirrors
; 2- loads in the LBC UIF the OB RB_rV40extra.ob
...
;X 7- sends a final, seeing dependent, sph3 correction after convergence has been reached.


; from the procedure (DOFPIA):
;X 0- reads config file lbcfpia.cfg
;X 0.5- checks that LBC is authorized
;X 1- sends initial +600mm sph3 offset to both mirrors
...
;
; last modified by J. M. Hill - 09-JUN-2014
;                (change name to dowrs)
;                (use smaller 52 vs 64 pupils for WRS)
; last modified by J. M. Hill - 14-JUN-2014
;                (use smaller 40 vs 64 pupils for WRS)
; last modified by J. M. Hill - 02-SEP-2014
;                (Confirm that LBC is the authorized instrument on
;                each side. Issue 5286)
; last modified by M. Stangalini - 03-Jul-2015
;                (make_wrs2 added to compile on the fly the necessary routines)
; last modified by J. M. Hill - 02-SEP-2016
;                (switch dowrs to the "low" focus readout position)
;                (merge dowrs and dofpia to make dohybrid)
...
Endif Else Begin
    ...
    SendSph3, Sph3, Red=Red, Sim=Sim, Logfile=Logfile
Endelse
Return
End
...
; Main Program
Pro DoHybrid, Sim=Sim, X2=X2, RedOnly=RedOnly, BlueOnly=BlueOnly, $
...
;
cfg = lbcfpia_read_cfg(ErrCode=ErrCode,ErrMsg=ErrMsg,/WRSMode)
...
; Force MaxIterations for WRS section of DoHybrid
MaxIterSave = MaxIterations
MaxIterations = 1  ; JUST ONE PASS OF WRS
...
;Print, HomeDir
DataDir = GetEnv("LBCFPIA_DATADIR")
;If DataDir EQ "" Then DataDir="/Repository"
If DataDir EQ "" Then DataDir="/TestRepo"


; Check that the requested side(s) of LBC is authorized.
If DoBlue GE 1 Then Begin
      Spawn, HomeDir+"/src/ddget.sh L_Instrument", Result, ErrResult
      Print, "Left Instrument is: ", Result
      If STRTRIM(Result,2) NE "LBC" Then Begin
         DoBlue = 0              ; Don't use Blue if it isn't authorized
         Print, "LBC-Blue is not authorized on the Left"
      EndIf
Endif
If DoRed GE 1 Then Begin
      Spawn, HomeDir+"/src/ddget.sh R_Instrument", Result, ErrResult
      Print, "Right Instrument is: ", Result
      If STRTRIM(Result,2) NE "LBC" Then Begin
         DoRed = 0              ; Don't use Red if it isn't authorized
         Print, "LBC-Red is not authorized on the Right"
      EndIf
Endif
If DoRed EQ 0 AND DoBlue EQ 0 Then Begin
   Print, "Neither side of LBC is authorized - stopping DOHYBRID."
   Return ; exit program
EndIf
...
BACK_Z11:
    If DoRed GE 1 Then Begin
        ...
        SendSph3, -RedInitial, /Red, Sim=Sim, Logfile=Logfile
    Endif
    If DoBlue GE 1 Then Begin
        Print, "Reversing ",-BlueInitial,"nm Sph3 (Z11) to BLUE channel"
        SendSph3, -BlueInitial, Sim=Sim, Logfile=Logfile
    Endif
    Return ; Exit Program
Endif
...
If ( KeyWord_Set(AskMe) AND Not KeyWord_Set(Sim) ) Then Begin
    Print, "DOWRS in /AskMe mode will prompt before sending Zernikes."
EndIf
...
If Not KeyWord_Set(Sim) Then Begin
  If (DoBlue EQ 0) AND (DoRed GE 1) Then Begin
      Print, "Loading OB ", HomeDir+"/src/load-ob40low-red.pl"
      Spawn, HomeDir+"/src/load-ob40low-red.pl", Result, ErrResult
  EndIf  
  If (DoBlue GE 1) AND (DoRed EQ 0) Then Begin
      Print, "Loading OB ", HomeDir+"/src/load-ob40low-blue.pl"
      Spawn, HomeDir+"/src/load-ob40low-blue.pl", Result, ErrResult
  EndIf
  If (DoBlue GE 1) AND (DoRed GE 1) Then Begin
      Print, "Loading OB ", HomeDir+"/src/load-ob40low.pl"
      Spawn, HomeDir+"/src/load-ob40low.pl", Result, ErrResult
  EndIf
...
; do LBCFPIA on Red
    If DoRed GE 1 Then Begin
        ...
        ;If StrLowCase(Response) EQ 'y' Then Begin
            ...
            Endif Else Begin
                Print, "Red LBCFPIA_WRS analysis failed."
...
; do LBCFPIA on Blue
    If DoBlue GE 1 Then Begin
        ...
        ;If StrLowCase(Response) EQ 'y' Then Begin
            ...
            Endif Else Begin
                Print, "Blue LBCFPIA_WRS analysis failed."
                If LoopCount LE 1 Then Begin
                    Print, "Going to backoff the initial Z11 and exit."
                    GOTO, BACK_Z11
                EndIf
...
; Send the final corrections (removed for WRS section of DOHYBRID)
Print, "DOWRS section of DOHYBRID is finished"





; Continuing with the DOFPIA section

; Read configuration parameters from file
print, "Reading the LBCFPIA config file"
; How to force this to read lbcfpia.cfg ? (suppress WRSMode? specify filename?)
cfg = lbcfpia_read_cfg(ErrCode=ErrCode,ErrMsg=ErrMsg, WRSMode="")
If ErrCode NE 0 Then Message, ErrMsg

; Check cfg file by looking at Z4 Limit
print, cfg.Z4Limit, "Z4 Limit"

; Check parameter for maximum number of iterations
If Not KeyWord_Set(MaxIterations) Then MaxIterations = cfg.DOFPIAMaxIter
If MaxIterations LE 0 Then MaxIterations = 1
; Restore parameter for maximum number of iterations to
;   replace the low value used for WRS
...
If Not KeyWord_Set(Sim) Then Begin
  If (DoBlue EQ 0) AND (DoRed GE 1) Then Begin
      Spawn, HomeDir+"/src/load-oblow-red.pl", Result, ErrResult
  EndIf  
  If (DoBlue GE 1) AND (DoRed EQ 0) Then Begin
      Spawn, HomeDir+"/src/load-oblow-blue.pl", Result, ErrResult
  EndIf
  If (DoBlue GE 1) AND (DoRed GE 1) Then Begin
      Spawn, HomeDir+"/src/load-oblow.pl", Result, ErrResult
  EndIf
...
; The following delay is needed before playing the OB if no pupils
; were found. Added by JMH 20140906
   WAIT, 2
...
; do LBCFPIA on Red
    If DoRed GE 1 Then Begin
        If Not KeyWord_Set(Sim) Then Begin
            If Keyword_Set(AskMe) Then Begin
                lbcfpia, /Red, ires=ires, estSeeing=RedSeeing, cfg=cfg
            Endif Else Begin
                lbcfpia, /Red, /ForceSend, ires=ires, estSeeing=RedSeeing, cfg=cfg
            Endelse
        ;Print, 'Are you satisfied with RED CHANNEL correction? (y/n)'
        ;Response = ''
        ;Read, Response
        ;If StrLowCase(Response) EQ 'y' Then Begin
            ...
            If N_ELEMENTS(ires) GT 0 Then Begin
                ...
                If abs(ires[10]) GT 0 Then BadZ11Red = 0
            Endif Else Begin
                Print, "Red LBCFPIA analysis failed."
                If LoopCount LE 1 Then Begin
                    Print, "Going to backoff the initial Z11 and exit."
                    GOTO, BACK_Z11
                EndIf
            EndElse
        Endif Else Begin
            lbcfpia, /Red, /DontSend, ires=ires, estSeeing=RedSeeing, cfg=cfg
            Print, "Simulation Checking for Red Convergence"
            If N_ELEMENTS(ires) GT 0 Then Begin
                If CheckAbLimits(ires, Limit=RedLimit, Seeing=RedSeeing) EQ 1 Then Begin
                    Print, "Red Convergence reached for seeing of ", RedSeeing, " arcsec."
...
; do LBCFPIA on Blue
    If DoBlue GE 1 Then Begin
        If Not KeyWord_Set(Sim) Then Begin
            If KeyWord_Set(AskMe) Then Begin
                lbcfpia, ires=ires, estSeeing=BlueSeeing, cfg=cfg
            Endif Else Begin
                lbcfpia, /ForceSend, ires=ires, estSeeing=BlueSeeing, cfg=cfg
            EndElse
        ;Print, 'Are you satisfied with BLUE CHANNEL correction? (y/n)'
        ;Response = ''
        ;Read, Response
        ;If StrLowCase(Response) EQ 'y' Then Begin
            ...
            If N_ELEMENTS(ires) GT 0 Then Begin
                ...
                If abs(ires[10]) GT 0 Then BadZ11Blue = 0
            Endif Else Begin
                Print, "Blue LBCFPIA analysis failed."
                If LoopCount LE 1 Then Begin
                    Print, "Going to backoff the initial Z11 and exit."
                    GOTO, BACK_Z11
                EndIf
            EndElse
        Endif Else Begin
            lbcfpia, /DontSend, ires=ires, estSeeing=BlueSeeing, cfg=cfg
            Print, "Simulation Checking for Blue Convergence"
            If N_ELEMENTS(ires) GT 0 Then Begin
                If CheckAbLimits(ires, Limit=BlueLimit, Seeing=BlueSeeing) EQ 1 Then Begin
                    Print, "Blue Convergence reached for seeing of ", BlueSeeing, " arcsec."
...
If DoRed GE 1 Then Begin
    ...
    Endif Else Begin
        ...
        SendSph3, Sph3, /Red, Sim=Sim, Logfile=Logfile
    EndElse
Endif
...
If DoBlue GE 1 Then Begin 
    ...
    Endif Else Begin
        ...
        SendSph3, Sph3, Sim=Sim, Logfile=Logfile
    EndElse
Endif

Print, "DOFPIA section of DOHYBRID is finished"
Return
End
